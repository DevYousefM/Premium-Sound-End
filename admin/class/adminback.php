...
class  adminback
{
    ...
    function add_product($data)
    {
        ...
        if (($img_ext == "jpg" || $img_ext == 'jpeg' || $img_ext == "png")) {
            if (($sound_ext == "mp3" || $sound_ext == 'm4a')) {
                ...
                $msg = "Sound file should be mp3 or m4a format";
                return $msg;
            }
        } else {
            $msg = "Image file should be jpg or png format";
            return $msg;
        }
    }
    ...
    function update_product($data)
    {
        ...
        if (($img_ext == "jpg" || $img_ext == 'jpeg' || $img_ext == "png") &&
            ...
            $msg = "Invalid image or sound format";
            return $msg;
        }
    }
    function update_coupon($data)
    {
        ...
        $coupon_name = $data['u_coupon_name'];
        $coupon_des = $data['u_coupon_desc'];
        $coupon_discount = $data['u_coupon_discount'];
        $coupon_status = $data['u_status'];

        $query = "UPDATE `cupon` SET `cupon_code`='$coupon_name',`description`='$coupon_des',`discount`='$coupon_discount',`status`=$coupon_status WHERE cupon_id=$coupon_id";
        if (mysqli_query($this->connection, $query)) {
    ...
    function edit_coupon($id)
    {
        ...
        if (mysqli_query($this->connection, $query)) {
            $coupon_info = mysqli_query($this->connection, $query);
            return $coupon_info;
        }
    }
    function delete_coupon($cmt_id)
    {
        $query = "DELETE FROM `cupon` WHERE `cupon_id`=$cmt_id";
