...
if (isset($_POST['action'])) {
    if ($_POST['action'] == 'load_accepted_order') {
        $date = $_POST['did'];
...
if (isset($_POST['action'])) {
    if ($_POST['action'] == 'sell_by_product') {
        ...
        $sell_query = "SELECT `order_id`, `product_name`, SUM(`amount`) FROM all_order_info WHERE (`order_date` BETWEEN '$date' and CURDATE()) GROUP BY `product_name`";
